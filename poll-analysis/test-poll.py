...
def print_results():
    ...
    print("-------------------")
    print(f"{percent_votes_khan}% (Total Votes: {len(votes['Khan'])})")
    print(f"{percent_votes_correy}% (Total Votes: {len(votes['Correy'])})")
    print(f"{percent_votes_li}% (Total Votes: {len(votes['Li'])})")
    print(f"{percent_votes_tooley}% (Total Votes: {len(votes['Tooley'])})")
    print("-------------------")
...
with open(polldata_path, mode='r', newline='') as polldata:
    ...
    for row in csvreader:
        votes["Khan"].append(row for row in csvreader if row[2] == "Khan")
        votes["Correy"] =[row for row in csvreader if row[2] == "Correy"]
        votes["Li"] = [row for row in csvreader if row[2] == "Li"]
        votes["Tooley"] = [row for row in csvreader if row[2] == "Tooley"]
